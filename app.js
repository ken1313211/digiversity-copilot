...
// Connection state is based on Firebase, not just the browser's network flag.
let databaseConnected = false;
let serverOffset = 0;
let latestSession = null;
let presenceListener = null;
let presenceDisconnect = null;
let presencePath = null;
...
function connectionMessage(message, failed = false) {
    ...
    banner.classList.toggle("connection-error", failed);
}
function syncError(error) {
    console.error("Live sync failed:", error);
    connectionMessage("Sync failed: " + (error?.message || error) + ". Check your connection and retry.", true);
}
function requireConnection() {
    if (!isFirebaseEnabled || !databaseConnected) throw new Error("Not connected to the game server");
}
async function transactRoom(roomRef, updater) {
    // A transaction's initial value can be null when its path is not cached.
    // Keep a read subscription alive until commit so null means a missing room.
    let unsubscribe;
    try {
        await new Promise((resolve, reject) => {
            unsubscribe = onValue(roomRef, () => resolve(), reject);
        });
        return await runTransaction(roomRef, updater, { applyLocally: false });
    } finally { if (unsubscribe) unsubscribe(); }
}
async function hostAction(action) {
    if (hostActionBusy) return;
    hostActionBusy = true;
    try { requireConnection(); await action(); } catch (error) { syncError(error); }
    finally { hostActionBusy = false; }
}
function startPresence() {
    const generation = ++presenceGeneration;
    if (presenceListener) presenceListener();
    if (presenceDisconnect) presenceDisconnect.cancel().catch(syncError);
    if (presencePath) remove(ref(database, presencePath)).catch(syncError);
    const pin = currentSessionPin;
    const key = myPlayerKey;
    const role = currentRole;
    const path = role === "host" ? `sessions/${pin}/hostConnections` : `sessions/${pin}/players/${key}/connections`;
    const connectionKey = push(ref(database, path)).key;
    presencePath = `${path}/${connectionKey}`;
    presenceListener = onValue(ref(database, ".info/connected"), async snapshot => {
        if (!snapshot.val()) return;
        try {
            const disconnect = onDisconnect(ref(database, `${path}/${connectionKey}`));
            presenceDisconnect = disconnect;
            await disconnect.remove();
            if (generation !== presenceGeneration || pin !== currentSessionPin || role !== currentRole) { await disconnect.cancel(); return; }
            // A kicked player or deleted room must never be recreated by reconnecting.
            await transactRoom(ref(database, `sessions/${pin}`), room => {
                if (!room || (role === "player" && !room.players?.[key])) return;
                const participant = role === "host" ? room : room.players[key];
                const field = role === "host" ? "hostConnections" : "connections";
                participant[field] ||= {};
                participant[field][connectionKey] = true;
                return room;
            }, { applyLocally: false });
...
function setupConnectionMonitoring() {
    if (!isFirebaseEnabled) { connectionMessage("Live game unavailable: Firebase failed to initialize.", true); return; }
    onValue(ref(database, ".info/serverTimeOffset"), snap => { serverOffset = Number(snap.val()) || 0; }, syncError);
    onValue(ref(database, ".info/connected"), snap => {
        databaseConnected = snap.val() === true;
        connectionMessage(databaseConnected ? "Connected to game server" : "Connection lost — reconnecting. Answers and host controls are unavailable.", !databaseConnected);
    }, syncError);
}
async function recoverPlayerSession() {
    const pin = sessionStorage.getItem("player_session_pin");
    const key = sessionStorage.getItem("player_key");
    if (!pin || !key || sessionStorage.getItem("player_role") !== "player") return;
    currentSessionPin = pin;
    myPlayerKey = key;
    myNickname = sessionStorage.getItem("player_nickname") || "Player";
    currentRole = "player";
    document.getElementById("display-player-name").innerText = myNickname;
    bindPlayerSessionSyncPipeline();
    startPresence();
}
async function submitPlayerAnswer(payload) {
    const pin = currentSessionPin;
    const key = myPlayerKey;
    const question = playerActiveQuestionIndex;
    const submittedAt = serverNow();
    connectionMessage("Sending answer…");
    try {
        requireConnection();
        const result = await transactRoom(ref(database, `sessions/${pin}`), room => {
            if (!room || room.status !== "question" || room.currentQuestion !== question ||
                !room.players?.[key] || room.timerPaused ||
                serverNow() > room.questionStartTime + (room.timeLimit || 20) * 1000 ||
                room.answers?.[key]) return;
            room.answers ||= {};
            room.answers[key] = { ...payload, questionIndex: question,
                elapsedTime: Math.max(0, submittedAt - room.questionStartTime) };
            return room;
        }, { applyLocally: false });
        if (!result.committed) throw new Error("Answer was already submitted, paused, or the question has closed");
        if (pin === currentSessionPin && question === playerActiveQuestionIndex && latestSession?.status === "question") {
            document.getElementById("player-waiting-msg").classList.remove("hidden");
            connectionMessage("Answer received by server");
        }
        return true;
    } catch (error) {
        if (pin === currentSessionPin && question === playerActiveQuestionIndex && latestSession?.status === "question" && !latestSession.answers?.[key]) {
            hasAnsweredCurrent = false;
            playerActiveQuestionIndex = -1;
            preparePlayerInputInterface(latestSession);
        }
        syncError(error);
        return false;
    }
...
function playSound(sound) {
    try { sound?.play()?.catch(() => {}); } catch (_) { /* Audio may require a user gesture. */ }
}
...
function purgeActiveListeners() {
    ...
    if (presenceDisconnect) { presenceDisconnect.cancel().catch(syncError); presenceDisconnect = null; }
    if (presencePath) { remove(ref(database, presencePath)).catch(syncError); presencePath = null; }
    latestSession = null;
    currentRole = null;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (isFirebaseEnabled) recoverPlayerSession().catch(syncError);
    const urlPin = new URLSearchParams(window.location.search).get("pin");
    if (/^\d{6}$/.test(urlPin || "") && !currentRole) {
...
function setupAdminAuthWorkflow() {
    ...
    loginForm?.addEventListener("submit", async (e) => {
        ...
        if (!isFirebaseEnabled) { syncError(new Error("Firebase is unavailable")); return; }
...
function renderQuizSelector() {
    ...
    listContainer.querySelectorAll(".btn-launch-quiz").forEach(btn => {
        btn.addEventListener("click", (e) => {
            ...
            hostAction(() => initializeLiveRoom(copilotPresets[idx]));
        });
    });
    ...
    document.getElementById("btn-start-game").onclick = () => hostAction(async () => {
        hostActiveQuestionIndex = 0;
        await executeQuestionBroadcast();
    });
    document.getElementById("btn-cancel-session").onclick = () => hostAction(terminateRoomInstance);
    document.getElementById("btn-host-end-game-early").onclick = () => hostAction(async () => {
        if (!confirm("End game early and skip to final results?")) return;
        clearInterval(timerInterval);
        hostActiveQuestionIndex = currentQuizData.questions.length;
        await presentHostLeaderboardView();
    });
    document.getElementById("btn-banner-rejoin").onclick = () => hostAction(recoverHostSession);
    document.getElementById("btn-banner-destroy").onclick = () => {
        if (confirm("Destroy the active live session?")) hostAction(terminateRoomInstance);
    };
...
async function initializeLiveRoom(quiz) {
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
        ...
        const result = await transactRoom(gameSessionRef, room => room === null ?
            { status: "lobby", quizTitle: quiz.title, currentQuestion: -1, timestamp: serverNow(), presenceVersion: 2 } : undefined,
            { applyLocally: false });
        if (result.committed) { claimed = true; break; }
    }
    if (!claimed) throw new Error("Could not reserve a game PIN. Please retry");
    ...
    if (qrContainer) {
        ...
        if (typeof QRCode !== "undefined") new QRCode(qrContainer, {
            text: `${window.location.origin}${window.location.pathname}?pin=${currentSessionPin}`,
...
function trackLobbyRegistrations() {
    ...
    playerLobbyListener = onValue(ref(database, `sessions/${currentSessionPin}/players`), (snapshot) => {
        ...
        const keys = Object.keys(data).filter(k => Object.keys(data[k].connections || {}).length > 0);
        ...
        keys.forEach(k => {
            ...
            tag.innerHTML = `<span>${escapeHTML(data[k].nickname)}</span><button class="btn-kick" data-key="${k}">×</button>`;
        ...
        listGrid.querySelectorAll(".btn-kick").forEach(btn => {
            btn.addEventListener("click", (e) => {
                ...
                hostAction(() => remove(ref(database, `sessions/${currentSessionPin}/players/${targetKey}`)));
            });
        });
    }, syncError);
...
async function terminateRoomInstance() {
    requireConnection();
    if (currentSessionPin) await remove(ref(database, `sessions/${currentSessionPin}`));
    purgeActiveListeners();
...
async function executeQuestionBroadcast(isReconnect = false) {
    ...
    if (isFirebaseEnabled) {
        if (!isReconnect) {
            const broadcast = await transactRoom(gameSessionRef, room => {
                if (!room || !((room.status === "lobby" && hostActiveQuestionIndex === 0) ||
                    (room.status === "leaderboard" && room.currentQuestion + 1 === hostActiveQuestionIndex))) return;
                return { ...room,
                status: "question", 
            ...
            if (!broadcast.committed) throw new Error("The host session has already moved to another question. Rejoin the session to sync.");
        }
    ...
    const snap = await get(gameSessionRef);
    latestSession = snap.val();
    runTimerCountdown();
}
...
function runTimerCountdown() {
    ...
    const tick = () => {
        if (!latestSession || latestSession.status !== "question") return;
        isTimerPaused = latestSession.timerPaused === true;
        timeLeft = Math.max(0, Math.ceil((isTimerPaused ? latestSession.pausedRemaining :
            latestSession.questionStartTime + (latestSession.timeLimit || 20) * 1000 - serverNow()) / 1000));
        timerUI.innerText = timeLeft;
        timerUI.classList.toggle("timer-warning", timeLeft <= 5);
        document.getElementById("btn-pause-timer").classList.toggle("hidden", isTimerPaused);
        document.getElementById("btn-resume-timer").classList.toggle("hidden", !isTimerPaused);
        if (timeLeft <= 0 && !isTimerPaused && databaseConnected && !evaluationBusy) {
            hostAction(concludeQuestionEvaluation);
        }
    };
    if (sessionStateListener) sessionStateListener();
    sessionStateListener = onValue(gameSessionRef, snap => {
        if (!snap.exists()) { purgeActiveListeners(); switchView("hostSetup"); return; }
        latestSession = snap.val();
        tick();
    }, syncError);
    timerInterval = setInterval(tick, 200);
    tick();
    document.getElementById("btn-pause-timer").onclick = () => hostAction(async () => {
        await update(gameSessionRef, { timerPaused: true, pausedRemaining: Math.max(0,
            latestSession.questionStartTime + (latestSession.timeLimit || 20) * 1000 - serverNow()) });
    });
    document.getElementById("btn-resume-timer").onclick = () => hostAction(async () => {
        await update(gameSessionRef, { timerPaused: false, questionStartTime:
            serverNow() - ((latestSession.timeLimit || 20) * 1000 - latestSession.pausedRemaining), pausedRemaining: null });
    });
    document.getElementById("btn-skip-question").onclick = () => hostAction(concludeQuestionEvaluation);
}
...
async function concludeQuestionEvaluation() {
    ...
    evaluationBusy = true;
    try {
    await evaluateSystemScoringTransactions(currentQuizData.questions[hostActiveQuestionIndex].correct);
    clearInterval(timerInterval);
    if (answersListener) { answersListener(); answersListener = null; }
    const q = currentQuizData.questions[hostActiveQuestionIndex];
    ...
    document.getElementById("btn-next-leaderboard").onclick = () => hostAction(presentHostLeaderboardView);
    } finally { evaluationBusy = false; }
}

async function evaluateSystemScoringTransactions(correctIdx) {
    const result = await transactRoom(gameSessionRef, room => {
    if (!room || room.currentQuestion !== hostActiveQuestionIndex) return;
    if (room.scoredQuestion === hostActiveQuestionIndex) return room;
    ...
    Object.keys(playersData).forEach(pKey => {
        const candidate = answers[pKey];
        const answerObj = candidate?.questionIndex === hostActiveQuestionIndex ? candidate : null;
        ...
        if (answeredCorrectly) {
            if (currentQ.type === "poll") {
                ...
                const scale = Math.max(0.2, Math.min(1, 1 - ((Number(answerObj.elapsedTime) || 0) / durationLimit)));
    ...
    for (const [path, value] of Object.entries(updates)) {
        ...
        room.players[key][field] = value;
    }
    room.scoredQuestion = hostActiveQuestionIndex;
    room.status = "results";
    room.totalAnswers = (room.totalAnswers || 0) + answersCount;
    room.totalCorrect = (room.totalCorrect || 0) + correctCount;
    return room;
    }, { applyLocally: false });
    if (!result.committed) throw new Error("Question changed before scoring completed");
    const room = result.snapshot.val();
    hostAnswersMap = room.answers || {};
    sessionTotalAnswersCount = room.totalAnswers || 0;
    sessionTotalCorrectAnswersCount = room.totalCorrect || 0;
}
...
async function presentHostLeaderboardView() {
    ...
    if (playersSnapshot.exists()) {
        ...
        array.slice(3, 10).forEach((player, idx) => {
            ...
            row.innerHTML = `<span>#${idx + 4} ${escapeHTML(player.nickname)}</span><span>${player.score} pts</span>`;
    ...
    if (hostActiveQuestionIndex + 1 < currentQuizData.questions.length) {
        ...
        nextBtn.onclick = () => hostAction(async () => {
            const previous = hostActiveQuestionIndex;
            hostActiveQuestionIndex++;
            try { await executeQuestionBroadcast(); } catch (error) { hostActiveQuestionIndex = previous; throw error; }
        });
        await update(gameSessionRef, { status: "leaderboard" });
    } else {
        nextBtn.classList.add("hidden");
        backBtn.classList.remove("hidden");
        backBtn.innerText = "Conclude Workshop Session";
        await update(gameSessionRef, { status: "gameover" });
        backBtn.onclick = () => hostAction(async () => {
            const playersCount = playersSnapshot.exists() ? Object.keys(playersSnapshot.val()).length : 0;
            const correctRatio = sessionTotalAnswersCount > 0 ? (sessionTotalCorrectAnswersCount / sessionTotalAnswersCount) : 0;
...
function setupPlayerParticipationWorkflow() {
    ...
    joinForm?.addEventListener("submit", async (e) => {
        ...
        try {
            submitBtn.disabled = true;
            requireConnection();
            if (!/^\d{6}$/.test(pinInput)) throw new Error("Enter a six-digit game PIN");
            ...
            if (!["lobby", "question", "results", "leaderboard"].includes(sessionSnap.val().status)) {
            ...
            const savedKey = sessionStorage.getItem("player_session_pin") === pinInput ? sessionStorage.getItem("player_key") : null;
            myPlayerKey = savedKey || push(playerListRef).key;
            const joined = await transactRoom(ref(database, `sessions/${pinInput}`), room => {
                if (!room || !["lobby", "question", "results", "leaderboard"].includes(room.status)) return;
                room.players ||= {};
                if (Object.entries(room.players).some(([key, p]) => key !== myPlayerKey && p.nickname.toLowerCase() === nameInput.toLowerCase())) return;
                room.players[myPlayerKey] ||= { nickname: myNickname, score: 0, streak: 0, lastPointsEarned: 0, wasCorrect: false, online: false };
                return room;
            }, { applyLocally: false });
            if (!joined.committed) throw new Error("Nickname is already taken, or this room has ended");
            sessionStorage.setItem("player_session_pin", pinInput);
            sessionStorage.setItem("player_key", myPlayerKey);
...
async function recoverHostSession() {
    ...
    if (savedPin && savedRole === "host" && savedQuiz) {
        ...
        try {
            ...
            startPresence();
            sessionTotalAnswersCount = session.totalAnswers || 0;
            sessionTotalCorrectAnswersCount = session.totalCorrect || 0;
            ...
            if (session.status === "lobby") {
                ...
                await concludeQuestionEvaluation();
...
function bindPlayerSessionSyncPipeline() {
    ...
    sessionStateListener = onValue(ref(database, `sessions/${currentSessionPin}`), (snapshot) => {
        ...
        currentScore = session.players[myPlayerKey].score || 0;
        currentStreak = session.players[myPlayerKey].streak || 0;
        if (session.status !== "question") clearInterval(window.potentialPointsInterval);
        if (session.presenceVersion === 2 && !Object.keys(session.hostConnections || {}).length) connectionMessage("Host disconnected — waiting for the host to reconnect.", true);
        else if (databaseConnected) connectionMessage(session.timerPaused ? "Host paused the question" : "Connected to game server");
        switch (session.status) {
            case "lobby": switchView("playerLobby"); break;
            case "question": preparePlayerInputInterface(session); break;
            case "results":
            case "leaderboard": renderPlayerResultPanel(session); break;
...
function preparePlayerInputInterface(session) {
    ...
    currentQuestionStartTime = session.questionStartTime || serverNow();
    if (session.answers?.[myPlayerKey]?.questionIndex === sessionQIndex) {
        playerActiveQuestionIndex = sessionQIndex;
        hasAnsweredCurrent = true;
        switchView("playerQuestion");
        document.getElementById("player-waiting-msg").classList.remove("hidden");
        document.querySelectorAll("#view-player-question button, #view-player-question input").forEach(el => {
            if (!el.classList.contains("btn-player-sync") && !el.classList.contains("btn-emoji")) el.disabled = true;
        });
        return;
    }
    if (playerActiveQuestionIndex !== sessionQIndex) {
        ...
        return; // Other players and reactions must not reset an in-progress answer.
    }
    currentQuestionStartTime = session.questionStartTime || serverNow();
    ...
    if(potentialPointsEl) {
        ...
        if(session.questionType !== "poll") {
            ...
            window.potentialPointsInterval = setInterval(() => {
                ...
                const elapsed = latestSession?.timerPaused ? (latestSession.timeLimit || 20) * 1000 - latestSession.pausedRemaining : serverNow() - currentQuestionStartTime;
    ...
    if (qType === "multiple-choice") {
        ...
        btns.forEach(btn => {
            btn.onclick = async (e) => {
                ...
                if (isFirebaseEnabled && myPlayerKey) {
                    await submitPlayerAnswer({ optionIndex: chosenIdx, elapsedTime: Date.now() - currentQuestionStartTime });
                }
            };
        });
    } else if (qType === "true-false") {
        ...
        submitBtn.onclick = async () => {
            ...
            submitBtn.disabled = true;
            if (isFirebaseEnabled && myPlayerKey) {
                await submitPlayerAnswer({ textAnswer: typedAns, elapsedTime: Date.now() - currentQuestionStartTime });
            }
        };
    } else if (qType === "number-guess") {
        ...
        submitBtn.onclick = async () => {
            ...
            submitBtn.disabled = true;
            if (isFirebaseEnabled && myPlayerKey) {
                await submitPlayerAnswer({ numberAnswer: parseFloat(rangeEl.value), elapsedTime: Date.now() - currentQuestionStartTime });
            }
        };
    } else if (qType === "poll") {
        ...
        options.forEach((opt, idx) => {
            ...
            btn.innerHTML = `<span style="font-weight:bold; color:var(--color-cyan)">${String.fromCharCode(65 + idx)}</span> <span>${escapeHTML(opt)}</span>`;
            btn.onclick = async () => {
                if (hasAnsweredCurrent) return;
                hasAnsweredCurrent = true;
                ...
                if (isFirebaseEnabled && myPlayerKey) {
                    await submitPlayerAnswer({ optionIndex: idx, elapsedTime: Date.now() - currentQuestionStartTime });
                }
            };
        ...
        submitBtn.onclick = async () => {
            ...
            submitBtn.disabled = true;
            if (isFirebaseEnabled && myPlayerKey) {
                await submitPlayerAnswer({ textAnswer: val, elapsedTime: Date.now() - currentQuestionStartTime });
            }
        };
    }
}
...
function renderPlayerJumbledPrompt(words) {
    ...
    const checkSubmission = async () => {
        if (constructedSequence.length === words.length) {
            ...
            if (isFirebaseEnabled && myPlayerKey) {
                await submitPlayerAnswer({ sequence: constructedSequence, elapsedTime: Date.now() - currentQuestionStartTime });
            }
        }
    };
    ...
    scrambled.forEach((w, index) => {
        ...
        chip.onclick = () => {
            if (hasAnsweredCurrent) return;
            if (chip.parentElement === constructedContainer) {
                availableContainer.appendChild(chip);
            } else {
                constructedContainer.appendChild(chip);
            }
            constructedSequence = [...constructedContainer.children].map(item => item.innerText);
            checkSubmission();
        };
        availableContainer.appendChild(chip);
    });
...
async function renderPlayerResultPanel(session) {
    ...
    try {
        const pData = session.players?.[myPlayerKey];
        if (!pData) return;
        ...
        const sortedPool = Object.entries(session.players).map(([k, v]) => ({ key: k, score: v.score })).sort((a, b) => b.score - a.score);
...
async function listUIFinalLeaderboard(session) {
    ...
    if (!isFirebaseEnabled) return;
    try {
        const playersSnapshot = { exists: () => !!session.players, val: () => session.players };
        ...
        if (playersSnapshot.exists()) {
            ...
            sortedIndices.forEach(idx => {
                ...
                row.innerHTML = `<span>#${idx + 1} ${escapeHTML(player.nickname)}</span><span>${player.score} pts</span>`;
                listUI.appendChild(row);
...
function setupPlayerReactionPipelines() {
    const triggerInboundReaction = async (emojiChar) => {
        if (emojiCooldownActive || !currentSessionPin || !databaseConnected) return;
